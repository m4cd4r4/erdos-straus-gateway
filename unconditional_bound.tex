...
Equivalently: among Case~B QR7 primes, $Q_A$-smoothness of $N_A$
characterises gateway~$A$ failure exactly.
\end{definition}
...
For $p \equiv 1, 4 \pmod{9}$ (two-thirds of cases) one has $v_3(N_{11}) = 1 < 2$,
so the required divisors $3^3 = 27$ and $3^4 = 81$ do not appear in $N_{11}^2$,
and the target $t_{11}$ need not be hit.  \textbf{Not sharp.}

\textbf{$A \ge 19$:}
For larger $A$, the order of any forced QR factor $g$ in $(\mathbb{Z}/A\mathbb{Z})^*$
is at least $(A-1)/2 \ge 9$, requiring $v_g(N_A) \ge (|Q_A|-1)/2 \ge 4$.
Direct inspection shows that for no $A \ge 19$ in the candidate list is the
forced multiplicity large enough to satisfy condition~(d)
simultaneously with conditions (a)--(c).  \textbf{Not sharp.}
\end{proof}
...
$10^9$  & 50,847,534 & 1,236,195 & $\le 239$ \\
$10^{10}$ & {\em in progress} & {\em ---} & $\le 239$ (expected) \\
\bottomrule
\end{tabular}
\end{center}

\begin{theorem}[GPU Verification]\label{thm:gpu}
The Erd\H{o}s--Straus conjecture holds for all primes
$p \le 10^{10}$, with maximum gateway parameter $A \le 239$.
No prime requiring $A > 239$ has been found.
\end{theorem}
