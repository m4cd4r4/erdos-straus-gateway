...
each Case~B QR7 prime using a finite set of candidate parameters
$A$, primes $\equiv 3 \pmod{4}$.
The original implementation used $28$ values $A \in \{3,7,11,\ldots,239\}$.
...
Section~\ref{sec:computation} reports an ongoing GPU-accelerated
verification to $10^{10}$ and a newly discovered prime
$p = 3{,}807{,}728{,}761$ which requires $A = 359$, necessitating
extension of the candidate set to $87$ values up to $A = 991$.
...
\begin{enumerate}
  ...
  \item GPU gateway kernel (one CUDA thread per Case~B QR7 prime):
    for each prime $p$, tries candidate $A$ values in order;
    for each $A$, factorises $N_A$ by trial division and builds
    the divisor residue set modulo~$A$.
\end{enumerate}

The original run used $28$ candidate $A$ values up to $A = 239$.
After discovering $p = 3{,}807{,}728{,}761$ requires $A = 359$
(Section~\ref{subsec:a359}), the kernel was corrected to use
$87$ candidate $A$ values up to $A = 991$ and the full run restarted.
...
$5 \times 10^9$   & $\approx 217{,}645{,}000$ & 5,378,137 & \textbf{359} (see \S\ref{subsec:a359}) \\
$10^{10}$ & {\em in progress (corrected run)} & {\em ---} & $\ge 359$ \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{Discovery: the first prime requiring $A > 239$}\label{subsec:a359}

An initial GPU run with the 28-candidate set ($A \le 239$) found
that the prime
\[
  p \;=\; 3{,}807{,}728{,}761
\]
is not solved by any gateway $A \le 239$.
...
\medskip
\noindent
This prime is the current record-holder: the largest gateway
parameter required for any prime up to $5 \times 10^9$.
The corrected GPU run with $87$ candidate $A$ values (up to $A = 991$)
is in progress and will determine whether further records occur
in $(5 \times 10^9, 10^{10})$.
...
while the maximum $A$ encountered can still grow: $A = 239$ suffices
to $10^9$, but $A = 359$ is required at $p = 3{,}807{,}728{,}761
\approx 3.8 \times 10^9$.
\end{remark}
...
\begin{question}
Extend the verification beyond $10^{10}$.  The corrected
87-candidate run (up to $A = 991$) is in progress; once complete,
extending to $10^{11}$ and beyond would determine whether $A = 359$
remains the hardest case, or whether larger gateway parameters
appear.  The maximum $A$ encountered for any prime $p \le X$
appears to grow slowly with $X$ (log-like); bounding this
growth rigorously is an open problem.
\end{question}
