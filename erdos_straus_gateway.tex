...
by Swett~\cite{Swett1999}.
Elsholtz and Tao~\cite{ElsholtzTao2013} proved that it holds for
``almost all'' positive integers in the sense of natural density.
Algebraic results of Schinzel~\cite{Schinzel1956},
Sierpi\'nski~\cite{Sierpinski1956}, Webb~\cite{Webb1970},
Mordell~\cite{Mordell1969}, Elsholtz~\cite{Elsholtz2001},
...
For every prime $p \equiv 1 \pmod{4}$ with $p \leq 10^9$,
there exists a prime $A \leq 239$ with $A \equiv 3 \pmod{4}$
...
Since primes $p \equiv 3 \pmod{4}$ are handled by the classical
identity (Proposition~\ref{prop:3mod4}) and composite~$n$
reduces to the prime case~\cite{Mordell1969},
Theorem~\ref{thm:main} implies that~\eqref{eq:es}
holds for all $n \leq 10^9$.
...
follows, since the finitely many gateway identities indexed by
...
$A$ fails has density zero---consistent with the ``almost all'' result
of Elsholtz and Tao~\cite{ElsholtzTao2013}.
...
The Erd\H{o}s--Straus conjecture reduces to:
...
Tenenbaum~\cite{Tenenbaum1984} on divisor distribution.

\subsection{Connection to prior work}

Our gateway decomposition can be viewed as making explicit the identities
whose existence is guaranteed by the Elsholtz--Tao density
argument~\cite{ElsholtzTao2013}.
While their result shows that the exceptional set has natural density zero
(see also Tao's survey~\cite{Tao2011}),
