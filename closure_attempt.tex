...
\begin{question}[Computation beyond $10^{10}$]\label{q:compute}
GPU verification~\cite{Gateway2026} has found $p = 3{,}807{,}728{,}761$
($\approx 3.8 \times 10^9$) requiring $A = 359$, extending the candidate
set to $87$ values up to $A = 991$.
Does any prime $p \in (5 \times 10^9, 10^{11})$ require $A > 359$?
The growth of $A^*(X)$ (the maximum gateway parameter needed for primes
$\le X$) remains a key open question; bounding it rigorously would
connect to the bounded-$A$ conjecture and make an explicit threshold
$P_0$ in Theorem~\ref{thm:conditional} computationally accessible.
\end{question}
...
\bibitem{Gateway2026}
M.~\'O~Murch\'u,
\emph{Bounded Gateway Parameters for the
Erd\H{o}s--Straus Conjecture}, preprint, 2026.
